...
exports.findAll = async(req, res, next) => {
    ...
    try {
        const document = await Books.find(condition) //.populate("tacgia", "name -_id")
        return res.send(document);
    } catch (error) {
        return next(
            new BadRequestError(
...
exports.findOne = async(req, res, next) => {
    ...
    try {
        const document = await Books.findOne(condition); //.populate("tacgia")
        if (!document) {
...
exports.update = async(req, res, next) => {
    ...
    try {
        const document = await Books.findOneAndUpdate(condition, req.body, {
            new: true,
        });
        if (!document) {
            return next(new BadRequestError(404, "Contact not found"));
...
exports.delete = async(req, res, next) => {
    ...
    try {
        const document = await Books.findOneAndDelete(condition);
        if (!document) {
            return next(new BadRequestError(404, "Contact not found"));
        }
